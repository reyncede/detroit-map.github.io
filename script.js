...
function showPopover(event) {
    ...
    if (!row.length) {
        ...
        var zillowLink = row.find('td:eq(2)').html().trim();
...
$('#regionTable tbody tr').each(function() {
    var aveValue = parseFloat($(this).find('td:nth-child(2)').text().replace('$', '').replace(',', ''));
    var badgeClass = '';

    if (aveValue < 70000) {
        badgeClass = 'badge-danger';
    } else if (aveValue < 100000) {
        badgeClass = 'badge-warning';
    } else if (aveValue < 200000) {
        badgeClass = 'badge-info';
    } else {
        badgeClass = 'badge-success';
    }

    var badgeElement = '<span class="badge ' + badgeClass + '">' + $(this).find('td:nth-child(2)').text() + '</span>';
